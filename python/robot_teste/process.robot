...
Execute um script e aguarde ele finalizar com sucesso
    ...
    ${MEU_PROCESSO}     Run Process     python   alunos_cornos.py
...
Execute um script e aguarde ele finalizar com falha
    ## Inicia o processo e espera pelo seu término
    ${MEU_PROCESSO}     Run Process     python   alunos_cornos_fail.py
    Log    Resultado da execução (sucesso/falha): ${MEU_PROCESSO.rc}
    Log    Saída da execução: ${MEU_PROCESSO.stdout}
...
Executando comandos diversos de prompt
    ...
    #                       ↑
    # Nesse caso vamos solicitar o robot para criar a pasta no diretório atual (cwd) ou 
    # diretorio corrente (CURDIR) do projeto. No meu caso o caminho é: 
    # C:\Users\guilh\Downloads\robot_framework\python\robot_teste\process.robot 

    # ${CURDIR} é uma variável do Robot Framework que retorna o caminho do diretório atual
    # e uma palavra reservada do Robot Framework.
    # Shell=True é um atributo do Robot Framework que habilita funções de shell, 
    # como criar um diretório, por exemplo. Habilita o uso de comandos do prompt.
    
*** Test Cases ***
Exemplo 01: Abrindo programas
    Abra e feche o Notepad
    # robot -d log -t "Exemplo 01: Abrindo programas" process.robot | Comando para abrir apenas esse processo
